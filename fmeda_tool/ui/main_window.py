...
class MainWindow(QMainWindow):
    ...
    def push_undo_state(self, action_desc: str):
        ...
        if self.current_project is not None:
            ...
            cur_view = "unit_editor"
            for v_name, v_widget in self.views.items():
                if self.stacked_widget.currentWidget() == v_widget:
                    cur_view = v_name
                    break
            cur_tab_idx = self.unit_editor_view.unit_tabs.currentIndex()
            self.undo_stack.append((project_dump, cur_view, cur_tab_idx, action_desc))
    ...
    def _on_undo(self):
        ...
        # Restore state on the relevant view without jumping pages
        cur_view = "unit_editor"
        for v_name, v_widget in self.views.items():
            if self.stacked_widget.currentWidget() == v_widget:
                cur_view = v_name
        ...
        if cur_view == "unit_editor":
            self.unit_editor_view.load_project(self.current_project)
            if 0 <= orig_tab_idx < self.unit_editor_view.unit_tabs.count():
                self.unit_editor_view.unit_tabs.setCurrentIndex(orig_tab_idx)
        elif cur_view == "verification":
            self.verification_view.load_project(self.current_project)
        elif cur_view == "export_view":
            self.export_view.load_project(self.current_project)
        else:
            self.unit_editor_view.load_project(self.current_project)
        
        if self.undo_stack:
            next_action_desc = self.undo_stack[-1][3]
            self.unit_editor_view.undo_btn.setEnabled(True)
