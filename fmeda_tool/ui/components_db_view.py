...
class ComponentsDBView(QWidget):
    ...
    def __init__(self, parent=None, project: Optional[Any] = None):
        super().__init__(parent)
        self.project = project
        self._setup_ui()
        self._load_exida_tab()
        self._load_legacy_tab()
        self._load_custom_tab()
    ...
    def _setup_ui(self):
        ...
        # 3. Custom Components Tab
        self.custom_tab = QWidget()
        self._setup_custom_ui(self.custom_tab)
        self.tabs.addTab(self.custom_tab, "Custom Components")
        
        # 4. Unassigned Display Names Tab (Batch Editor)
    ...
    # ---------------- 3. CUSTOM COMPONENTS TAB ----------------
    def _setup_custom_ui(self, parent: QWidget):
        layout = QVBoxLayout(parent)
        layout.setContentsMargins(10, 10, 10, 10)
        layout.setSpacing(10)
        
        # Filter Bar
        filter_box = QGroupBox("Filter Custom Components")
        filter_layout = QHBoxLayout(filter_box)
        filter_layout.setSpacing(10)
        ...
        self.custom_type_combo.currentIndexChanged.connect(self._load_custom_tab)
        filter_layout.addWidget(self.custom_type_combo, stretch=1)
        
        refresh_btn = QPushButton("Refresh")
        refresh_btn.clicked.connect(self._load_custom_tab)
        filter_layout.addWidget(refresh_btn)
        
        new_btn = QPushButton("➕ New Custom Component")
        ...
        """)
        new_btn.clicked.connect(self._on_new_custom_clicked)
        ...
        self.custom_count_lbl = QLabel("0 custom components")
        layout.addWidget(self.custom_count_lbl)

    def _load_custom_tab(self):
        # Refresh type combo
        filters = ComponentLibraryService.get_distinct_filter_values()
        curr_t = self.custom_type_combo.currentText()
        self.custom_type_combo.blockSignals(True)
        self.custom_type_combo.clear()
        self.custom_type_combo.addItem("All")
        self.custom_type_combo.addItems(filters["component_types"])
        self.custom_type_combo.setCurrentText(curr_t if curr_t in filters["component_types"] else "All")
        self.custom_type_combo.blockSignals(False)
        
        query = self.custom_search_input.text().strip()
        comp_type = self.custom_type_combo.currentText()
        
        results = ComponentLibraryService.search_custom_components(
            query=query,
            component_type=comp_type if comp_type != "All" else None,
            include_retired=True
        )
        ...
        for comp in results:
            ...
            if not dname:
                item_dname.setForeground(QColor("#6c757d"))
            self.custom_table.setItem(row, 0, item_dname)
            ...
            self.custom_table.setItem(row, 4, QTableWidgetItem(f"{fm_count} modes"))
            
            stat_item = QTableWidgetItem(comp["status"])
            stat_item.setForeground(QColor("#198754" if comp["status"] == "active" else "#dc3545"))
            self.custom_table.setItem(row, 5, stat_item)
            ...
            edit_btn.setStyleSheet("padding: 2px 8px; font-size: 11px;")
            edit_btn.clicked.connect(lambda _, c=comp: self._on_edit_custom_clicked(c))
            act_layout.addWidget(edit_btn)
            
            del_btn = QPushButton("Delete")
            del_btn.setStyleSheet("background-color: #dc3545; color: white; padding: 2px 8px; font-size: 11px; font-weight: bold; border-radius: 3px;")
            del_btn.clicked.connect(lambda _, c=comp: self._on_delete_custom_clicked(c))
            act_layout.addWidget(del_btn)
            
            self.custom_table.setCellWidget(row, 6, action_widget)
            
        self.custom_count_lbl.setText(f"Showing {len(results)} custom components")
    ...
    def _on_edit_custom_clicked(self, comp: Dict[str, Any]):
        dlg = CustomComponentDialog(custom_comp=comp, parent=self)
        if dlg.exec() == QDialog.DialogCode.Accepted:
            self._load_custom_tab()
            self._load_changelog_tab()

    def _on_delete_custom_clicked(self, comp: Dict[str, Any]):
        c_id = comp.get("id") or comp.get("library_component_id")
        if not c_id:
            return
            
        disp_lbl = comp.get("display_label") or comp.get("display_name") or comp.get("component_type") or c_id
        
        # 1. Reference check
        active_proj = self._get_active_project()
        is_ref, ref_msg, _ = ComponentLibraryService.is_custom_component_referenced(
            c_id, project=active_proj
        )
        if is_ref:
            QMessageBox.warning(
                self,
        ...
        if msg_box.clickedButton() == del_btn:
            success, msg = ComponentLibraryService.delete_custom_component(c_id)
            if not success:
    ...
    def _toggle_custom_status(self, comp: Dict[str, Any]):
        new_status = "retired" if comp["status"] == "active" else "active"
        success, msg = ComponentLibraryService.set_component_status(
            component_id=comp["id"],
            new_status=new_status,
            reason=f"Admin toggled custom component status to {new_status}"
        )
        if success:
            self._load_custom_tab()
            self._load_changelog_tab()
    ...
    # ---------------- 4. UNASSIGNED DISPLAY NAMES (BATCH EDIT) ----------------
