...
class ProjectOverviewTab(QScrollArea):
    ...
    def refresh(self, project: Project):
        ...
        add_rel_row("Selected Environmental Profile:", selected_prof, 1, 0)
...
class FunctionalGroupTab(QWidget):
    ...
    def _setup_ui(self, timer: Optional[PerformanceTimer] = None):
        ...
        self.import_txt_btn = QPushButton("Import BOM TXT")
        ...
        for btn in [
            self.add_comp_type_btn, self.add_bom_man_btn, self.import_txt_btn,
        ...
        self.import_txt_btn.clicked.connect(self._on_import_bom_txt)
        ...
        # QTableView and FmedaTableModel (37 Columns)
    ...
    def _on_map_bom_components(self):
        if not getattr(self.unit, "bom_components", None):
            QMessageBox.warning(self, "No BOM Imported", "Please import a BOM TXT file first before mapping components.")
            return
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            if self.main_editor and hasattr(self.main_editor, "main_window") and self.main_editor.main_window:
                self.main_editor.main_window.push_undo_state(f"Map BOM Batch: {self.unit.name}")
            ...
            QMessageBox.information(
                self, "Mappings Saved",
                f"BOM component mappings have been saved and FMEDA rows generated successfully for '{self.unit.name}'!"
            )
