    ...
    try:
        with get_db_connection(path) as conn:
            ...
            required = {
                ...
                "legacy_components", "legacy_failure_modes", "custom_components",
                "custom_failure_modes", "component_change_log"
            }
            if not required.issubset(tables):
                # If custom_components is the only missing piece in an otherwise valid DB, run schema script to create it
                base_required = {
                    "library_metadata", "profiles", "components", "legacy_components"
                }
                if base_required.issubset(tables):
                    with open(get_schema_sql_path(), "r", encoding="utf-8") as sf:
                        cur.executescript(sf.read())
                    tables = {row[0] for row in cur.execute(
                        "SELECT name FROM sqlite_master WHERE type='table';"
                    ).fetchall()}
            # Ensure custom_components has traceability columns
            if "custom_components" in tables:
                ...
                if "copied_from_component_id" not in col_names:
                    cur.execute("ALTER TABLE custom_components ADD COLUMN copied_from_component_id TEXT;")
                if "copied_from_failure_rate_id" not in col_names:
                    cur.execute("ALTER TABLE custom_components ADD COLUMN copied_from_failure_rate_id TEXT;")
                if "copied_at" not in col_names:
                    cur.execute("ALTER TABLE custom_components ADD COLUMN copied_at TEXT;")
