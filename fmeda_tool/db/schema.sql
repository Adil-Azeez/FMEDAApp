...
CREATE TABLE IF NOT EXISTS custom_components (
    ...
    fits REAL NOT NULL,
    status TEXT DEFAULT 'active' CHECK (status IN ('active', 'retired')),
    copied_from_source_type TEXT,
    copied_from_component_id TEXT,
    copied_from_failure_rate_id TEXT,
    copied_at TEXT,
    created_at TEXT NOT NULL,
    updated_at TEXT NOT NULL
);

-- 11. Custom Failure Modes
CREATE TABLE IF NOT EXISTS custom_failure_modes (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    custom_component_id TEXT NOT NULL REFERENCES custom_components(id) ON DELETE CASCADE,
    name TEXT NOT NULL,
    percentage REAL NOT NULL
);

-- 12. Component Change Log (Audit Trail)
...
CREATE INDEX IF NOT EXISTS idx_custom_components_display_name ON custom_components(display_name);
CREATE INDEX IF NOT EXISTS idx_custom_components_type ON custom_components(component_type);
CREATE INDEX IF NOT EXISTS idx_custom_fm_comp ON custom_failure_modes(custom_component_id);
