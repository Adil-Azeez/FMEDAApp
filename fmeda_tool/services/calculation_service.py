...
class CalculationService:
    ...
    def calculate_component(component: Component) -> Dict[str, Any]:
        ...
        """
        # Unmapped mapping templates do not contribute to FMEDA calculations
        if component.position in ("Mapping Template", "Template") or getattr(component, "is_template", False):
            return {
                "total_failure_rate": 0.0,
                "relevant_failure_rate": 0.0,
                "safe_failure_rate": 0.0,
                "dangerous_detected_rate": 0.0,
                "dangerous_undetected_rate": 0.0,
                "lambda_sd": 0.0,
                "lambda_su": 0.0,
                "lambda_no_part": 0.0,
                "lambda_no_effect": 0.0,
                "gesamtgerat": {
                    "lambda": 0.0, "lambda_safe": 0.0, "lambda_dangerous": 0.0,
                    "lambda_sd": 0.0, "lambda_su": 0.0, "lambda_dd": 0.0, "lambda_du": 0.0
                },
                "sicherheitskanal": {
                    "lambda": 0.0, "lambda_safe": 0.0, "lambda_dangerous": 0.0,
                    "lambda_sd": 0.0, "lambda_su": 0.0, "lambda_dd": 0.0, "lambda_du": 0.0
    ...
    def calculate_scope(units: List[Unit], project: Optional[Project] = None) -> Dict[str, Any]:
        ...
        for unit in units:
            ...
            for c in unit.components:
                if c.position in ("Mapping Template", "Template") or getattr(c, "is_template", False):
                    continue
                gg_comp_count += 1
    ...
    @staticmethod
    def calculate_unit_metrics(unit: Unit, project: Optional[Project] = None) -> Dict[str, Any]:
        """
        Calculates complete independent safety metrics for a single functional group
        without modifying the underlying project.
        """
        scope_res = CalculationService.calculate_scope([unit], project)
        gg = scope_res["gesamtgerat"]
        sk = scope_res["sicherheitskanal"]
        
        # MTBF (years) for overall device
        gg_fit = gg["lambda"]
        mtbf_years = (10**9 / (gg_fit * 8760.0)) if gg_fit > 0.0 else None
        
        # Proof test coverages across unit's dangerous undetected failures
        sum_ldu = 0.0
        sum_pt_a = 0.0
        sum_pt_b = 0.0
        sum_pt_c = 0.0
        ...
        for c in unit.components:
            if c.position in ("Mapping Template", "Template") or getattr(c, "is_template", False):
                continue
            comp_fit = c.failure_rate or 0.0
            for a in c.failure_mode_assignments:
                fm_pct = c.failure_modes.get(a.failure_mode_name, 0.0)
                local_fit = comp_fit * (fm_pct / 100.0)
                dp = a.dangerous_failure_percentage if a.dangerous_failure_percentage is not None else 100.0
                det = a.detection_percentage if a.detection_percentage is not None else 0.0
                ld = local_fit * (dp / 100.0)
                ldu = ld * (1.0 - det / 100.0)
                
                pt_a = getattr(a, "proof_test_a", 0.0) or 0.0
                pt_b = getattr(a, "proof_test_b", 0.0) or 0.0
                pt_c = getattr(a, "proof_test_c", 0.0) or 0.0
                
                sum_ldu += ldu
                sum_pt_a += ldu * (pt_a / 100.0)
                sum_pt_b += ldu * (pt_b / 100.0)
                sum_pt_c += ldu * (pt_c / 100.0)
                
        pt_cov_a = (sum_pt_a / sum_ldu * 100.0) if sum_ldu > 0.0 else None
        pt_cov_b = (sum_pt_b / sum_ldu * 100.0) if sum_ldu > 0.0 else None
        pt_cov_c = (sum_pt_c / sum_ldu * 100.0) if sum_ldu > 0.0 else None
        
        is_safety = bool(unit.included_in_safety_function)
        
        # SFF and DC (None if denominator is zero)
        gg_sff = gg["sff"]
        gg_dc = gg["dc"]
        sk_sff = sk["sff"] if is_safety else None
        sk_dc = sk["dc"] if is_safety else None
        sk_mttfd = sk["mttfd"] if is_safety else None
        
        pfd_avg = sk["pfd_avg"] if is_safety else None
        pfd_max = sk["pfd_max"] if is_safety else None
        ...
        return {
            ...
            # Gesamtgerät
            "lambda_total_gesamtgerat": gg["lambda"],
            "lambda_safe_gesamtgerat": gg["lambda_safe"],
            "lambda_dangerous_gesamtgerat": gg["lambda_dangerous"],
            "lambda_sd_gesamtgerat": gg["lambda_sd"],
            "lambda_su_gesamtgerat": gg["lambda_su"],
            "lambda_dd_gesamtgerat": gg["lambda_dd"],
            "lambda_du_gesamtgerat": gg["lambda_du"],
            "sff_gesamtgerat": gg_sff,
            "dc_gesamtgerat": gg_dc,
            "mtbf_years": mtbf_years,
            # Sicherheitskanal
            "lambda_total_sicherheitskanal": sk["lambda"] if is_safety else 0.0,
            "lambda_safe_sicherheitskanal": sk["lambda_safe"] if is_safety else 0.0,
            "lambda_dangerous_sicherheitskanal": sk["lambda_dangerous"] if is_safety else 0.0,
            "lambda_sd_sicherheitskanal": sk["lambda_sd"] if is_safety else 0.0,
            "lambda_su_sicherheitskanal": sk["lambda_su"] if is_safety else 0.0,
            "lambda_dd_sicherheitskanal": sk["lambda_dd"] if is_safety else 0.0,
            "lambda_du_sicherheitskanal": sk["lambda_du"] if is_safety else 0.0,
            "sff_sicherheitskanal": sk_sff,
            "dc_sicherheitskanal": sk_dc,
            "mttfd_sicherheitskanal": sk_mttfd,
            "pfd_avg": pfd_avg,
            "pfd_max": pfd_max,
            "achieved_sil": achieved_sil,
            # Proof tests
            "proof_test_a_coverage": pt_cov_a,
            "proof_test_b_coverage": pt_cov_b,
            "proof_test_c_coverage": pt_cov_c,
        }
